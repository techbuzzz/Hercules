...
public class HerculesBusTests
{
   ...
   public async Task EnsureChannel_Creates_And_Dedups()
   {
      ...
      var ch1 = await bus.EnsureChannelAsync("main", "Main", false, "alice");
      var ch2 = await bus.EnsureChannelAsync("main", "duplicate", true, "bob");
   ...
   public async Task Multiple_Subscribers_All_Receive_Broadcast()
   {
      ...
      await using var s1 = bus.Subscribe("test", async (m, _) =>
      {
         lock (received1)
         {
            received1.Add(m);
      ...
      await using var s2 = bus.Subscribe("test", async (m, _) =>
      {
         lock (received2)
         {
            received2.Add(m);
   ...
   public async Task Subscribe_Does_Not_Receive_Other_Channels()
   {
      ...
      var received = new List<AgentMessage>();
      await using var sub = bus.Subscribe("a", async (m, _) =>
      {
         received.Add(m);
         await ValueTask.CompletedTask;
      });
   ...
   public async Task SubscribeAll_Receives_From_All_Channels()
   {
      ...
      var received = new List<AgentMessage>();
      await using var sub = bus.SubscribeAll(async (m, _) =>
      {
         received.Add(m);
         await ValueTask.CompletedTask;
      });
   ...
   public async Task GetRecentAsync_Returns_Ordered_By_Time()
   {
      ...
      var recent = await bus.GetRecentAsync("test", 3);
   ...
   public async Task ListAgents_Filters_Offline()
   {
      ...
      var all = await bus.ListAgentsAsync();
      Assert.Equal(2, all.Count);

      var onlineOnly = await bus.ListAgentsAsync(false);
