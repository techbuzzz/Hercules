...
public sealed class PassthroughCompiler : IWasmCompiler
{
    ...
    public Task<byte[]> CompileAsync(string sourceCode, CancellationToken ct = default)
    {
        ...
        if (IsHex(trimmed))
        {
            try
            {
                return Task.FromResult(Convert.FromHexString(trimmed));
            }
    ...
    private static bool IsHex(string s)
    {
        ...
        return s.StartsWith("0061736d", StringComparison.OrdinalIgnoreCase) || (s.Length % 2 == 0 && s.All(c => "0123456789abcdefABCDEF".Contains(c)) && s.Length > 8);
    }
}
